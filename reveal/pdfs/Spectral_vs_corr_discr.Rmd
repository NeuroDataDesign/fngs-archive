---
title: "Spectral vs Correlational Discriminability"
...
### Discriminability as a function of Covariance of Noise given a Spectral Model
...
```{r, fig.height=6, fig.width=12, message=FALSE}  
  ...
  mnr_sims <- data.frame(cov=c(), mnr=c())
  ids <- ceiling(1:40/2)
  ...
  for (i in 1:length(cov_sim)) {
    ...
    div <- freq2div(amp_sig)
    dist <- div2dist(div)
    mnr_sims <- rbind(mnr_sims, data.frame(cov=cov_sim[i], mnr=mnr(rdf(dist, as.character(ids)))))
  }

  mnr_noise_sim_plot <- ggplot(data = mnr_sims, aes(x=cov, y=mnr)) +
    geom_line() +
...
```{r, fig.height=6, fig.width=12, message=FALSE}
  ...
  inpath <- 'C:/Users/ebrid/Documents/R/HNUoutputs_114/ts_outputs_HNU1/aal-2mm/'
...
```{r, fig.height=6, fig.width=12, message=FALSE}
  ...
  distance_plot <- ggplot(melt(distmtx), aes(x=Var1, y=Var2, fill=value)) + 
    ...
    xlab("Scan") +
    ylab("Scan") +
    ggtitle(sprintf('Ranked Spectral Approach Amplitude Spec, d=%.4f', mnrspec))
  kde_plot <- ggplot() +
    geom_ribbon(data=meltkde, aes(x=distance, ymax=Probability, fill=Relationship), ymin=0, alpha=0.5) +
    ggtitle('Subject Relationships')
  multiplot(distance_plot, kde_plot, layout=matrix(c(1,2), nrow=1, byrow=TRUE))
...
```{r, fig.height=6, fig.width=12, message=FALSE}
  ...
  Dranked <- distance(ranked_graphs)
  ...
  distance_plot <- ggplot(melt(Dranked), aes(x=Var1, y=Var2, fill=value)) + 
    geom_tile() +
    scale_fill_gradientn(colours=c("darkblue","blue","purple","green","yellow"),name="distance") +
    xlab("Scan") + ylab("Scan") + ggtitle(sprintf('Ranked Discriminability Approach, d=%.4f', mnrranked))
  kde_plot <- ggplot() +
    geom_ribbon(data=meltkde, aes(x=distance, ymax=Probability, fill=Relationship), ymin=0, alpha=0.5) +
    ggtitle('Subject Relationships')
  multiplot(distance_plot, kde_plot, layout=matrix(c(1,2), nrow=1, byrow=TRUE))
